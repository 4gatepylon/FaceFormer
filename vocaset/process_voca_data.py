...
def generate_vertices_npy(args, face_vert_mmap, data2array_verts):
    ...
    for sub in data2array_verts.keys():
        for seq in data2array_verts[sub].keys():
            ...
            np.save(
                os.path.join(args.vertices_npy_path, vertices_npy_name), vertices_npy
            )
...
def generate_wav(args, raw_audio):
    ...
    for sub in raw_audio.keys():
        for seq in raw_audio[sub].keys():
            wav_name = sub + "_" + seq
            wavfile.write(
                os.path.join(args.wav_path, wav_name + ".wav"),
                raw_audio[sub][seq]["sample_rate"],
                raw_audio[sub][seq]["audio"],
            )
...
def main():
    ...
    # Input
    parser.add_argument(
        "--data2array_verts_path", type=str, default="subj_seq_to_idx.pkl"
    )
